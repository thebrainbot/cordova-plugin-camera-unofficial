...
| localOnly | <code>Boolean</code> | | <code>FALSE</code> | Choose to use only local file sources. This is only for Android, it removes the Google Doc options from the selection menu. |
...
| IMAGEORVIDEO | <code>number</code> | <code>3</code> | Allow selection of only mime types image/* and video/* (Android only). |
...
If the user selects an image from Google Docs, the app will crash.
Setting localOnly to true will hide Google Docs.  This also helps as selections
from Google Docs aren't processible anyway.
